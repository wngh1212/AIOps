...
class ComparisonDashboard:
    ...
    def plot_grouped_comparison(self, output_file="01_grouped_comparison.png"):
        ...
        ax.set_title(
            "Clear Comparison of 4 Models: 4 Evaluation Metrics\n(Accuracy, JSON Valid, Speed, Consistency)",
            fontsize=16,
            fontweight="bold",
            pad=20,
        )
    ...
    def plot_overall_ranking(self, output_file="02_overall_ranking.png"):
        ...
        sorted_models = sorted(
            self.model_stats.items(), key=lambda x: x[1]["overall_score"], reverse=True
        )
        ...
        bars = ax.barh(
            y_pos, scores, color=colors_rank, height=0.6, edgecolor="white", linewidth=3
        )
        ...
        ax.set_title(
            "Overall Ranking (Accuracy 40% + JSON 30% + Speed 20% + Consistency 10%)",
            fontsize=16,
            fontweight="bold",
            pad=20,
        )
    ...
    def plot_metrics_scorecard(self, output_file="03_metrics_scorecard.png"):
        ...
        for category, description, key in metrics:
            ...
            for model in models:
                ...
                if (
                    "latency" in key.lower()
                    or key == "test_count"
                    or key == "success_count"
                ):
        ...
        ax.set_title(
            "Detailed Metrics Scorecard", fontsize=18, fontweight="bold", pad=20
        )
    ...
    def plot_category_matrix(self, output_file="04_category_matrix.png"):
        ...
        for model in models:
            row = [
                self.model_stats[model]["category_accuracy"].get(cat, 0)
        ...
        ax.set_title(
            "Accuracy Comparison by Category (All Models x All Categories)",
            fontsize=16,
            fontweight="bold",
            pad=20,
        )
    ...
    def plot_spider_comprehensive(self, output_file="05_spider_comprehensive.png"):
        ...
        ax.set_title(
            "Comprehensive Comparison of 4 Models (Accuracy, JSON, Speed, Consistency)",
            fontsize=18,
            fontweight="bold",
            pad=30,
        )
    ...
    def plot_summary_report(self, output_file="06_summary_report.png"):
        ...
        ax_title.text(
            0.5,
            0.7,
            "AIOps LLM Benchmark Final Report",
            ha="center",
            va="center",
            fontsize=28,
            fontweight="bold",
        ...
        ax_title.text(
            0.5,
            0.2,
            f"Analysis of Total {len(self.df)} Test Results",
            ha="center",
            va="center",
            fontsize=14,
            color="gray",
        ...
        bars = ax1.barh(
            y_pos, scores, color=colors_rank, edgecolor="white", linewidth=2
        )
        ...
        ax2.bar(
            ...
            accuracy_vals,
            color=["#2ecc71", "#3498db", "#f39c12", "#e74c3c"][: len(sorted_models)],
            edgecolor="white",
            linewidth=2,
            alpha=0.8,
        )
        ...
        ax2.set_xticklabels(
            [n.split()[0] for n in names], fontsize=10, fontweight="bold"
        )
        ...
        colors_latency = [
            "#e74c3c" if v > 2500 else "#f39c12" if v > 2000 else "#2ecc71"
            for v in latency_vals
        ]
        ax3.bar(
            range(len(sorted_models)),
            latency_vals,
        ...
        for i, v in enumerate(latency_vals):
            ax3.text(
                i, v + 50, f"{v:.0f}ms", ha="center", fontweight="bold", fontsize=11
            )
        ...
        ax3.set_title(
            "Average Latency (Lower is Better)", fontsize=13, fontweight="bold", pad=10
        )
        ...
        ax4.set_title(
            "JSON Validity (30% Weight)", fontsize=13, fontweight="bold", pad=10
        )
        ax4.set_ylim([0, 105])
    ...
    def print_text_report(self):
        ...
        for i, (model, stats) in enumerate(sorted_models, 1):
            ...
            print(
                f"   Success Rate: {stats['success_count']}/{stats['test_count']} tests"
            )
            print()
