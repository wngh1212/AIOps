...
TEST_CASES = [
    ...
    ("test_006", "instance", "Terminate i-0123456789abcdef", "terminate_resource", ["instance_id"]),
    ("test_007", "instance", "Delete the web-server instance", "terminate_resource", ["name"]),
    ("test_008", "instance", "Resize i-0123456789abcdef to t2.large", "resize_instance", ["instance_id", "instance_type"]),
    ("test_009", "instance", "Change instance type of my-server to t3.xlarge", "resize_instance", ["name", "instance_type"]),
    ("test_010", "instance", "Start web-server", "execute_aws_action", ["action_name"]),
    ("test_011", "instance", "Stop the database instance", "execute_aws_action", ["action_name"]),
    ("test_012", "instance", "Reboot i-abcd1234", "execute_aws_action", ["action_name"]),
    ("test_013", "instance", "Create a snapshot of i-0123456789abcdef", "create_snapshot", ["instance_id"]),
    ...
    ("test_022", "network", "Display the infrastructure layout", "generate_topology", []),
    ...
    ("test_031", "monitoring", "Show metrics for i-abc123", "get_metric", ["instance_id"]),
    ("test_032", "monitoring", "Monitor web-server performance", "get_metric", ["name"]),
    ("test_033", "monitoring", "Instance CPU usage", "get_metric", []),
    ("test_034", "monitoring", "Get performance metrics of web server", "get_metric", ["name"]),
    ("test_035", "monitoring", "What is i-0123456789abcdef doing", "get_metric", ["instance_id"]),
    ("test_036", "monitoring", "Check instance health", "get_metric", []),
    ("test_037", "monitoring", "Get logs from instance", "get_recent_logs", []),
    ("test_038", "monitoring", "Show recent logs of i-xyz", "get_recent_logs", ["instance_id"]),
    ("test_039", "monitoring", "Retrieve logs from web-server", "get_recent_logs", ["name"]),
    ...
    ("test_052", "cost", "Which instance uses the most resources", "analyze_resource_usage", []),
    ...
    ("test_060", "complex", "Resize instance and monitor performance", "resize_instance", ["instance_type"]),
    ("test_061", "complex", "Setup new VPC with instance", "create_vpc", []),
    ("test_062", "complex", "Terminate old instance and create new one", "terminate_resource", []),
    ...
    ("test_065", "complex", "Create instance for testing then generate topology", "create_instance", ["instance_type"]),
    ("test_066", "complex", "Scale infrastructure: list instances then resize", "list_instances", []),
    ...
    ("test_076", "edge", "!@#$%^&*()", None, []),
    ("test_077", "edge", "Create instance with invalid type xxx99", "create_instance", []),
    ("test_078", "edge", "Resize i-notreal to t2.micro", "resize_instance", ["instance_id", "instance_type"]),
    ("test_079", "edge", "Get metric for nonexistent-server", "get_metric", ["name"]),
    ("test_080", "edge", "Terminate invalid-id", "terminate_resource", []),
    ("test_081", "edge", "Create VPC with invalid CIDR 999.999.999.999", "create_vpc", ["cidr"]),
    ...
    ("test_088", "edge", "Very long prompt: " + "create instance " * 20, "create_instance", []),
    ("test_089", "edge", "Create-Instance (with hyphens)", "create_instance", []),
    ("test_090", "edge", "LAUNCH T2.MICRO UPPERCASE", "create_instance", ["instance_type"]),
    ("test_091", "edge", "terminate_resource i-123", "terminate_resource", ["instance_id"]),
    ("test_092", "edge", "list_instances status=running", "list_instances", []),
    ("test_093", "edge", 'JSON format: {"tool": "create_instance", "args": {"instance_type": "t2.nano"}}', "create_instance", ["instance_type"]),
]
...
[/INST]"""
    ...
    def measure_response(self, prompt: str) -> Tuple[str, Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"LLM 오류: {response.status_code}")
                return "", {"latency_ms": total_latency, "tokens_per_sec": 0, "success": False}
            ...
            total_latency = (time.perf_counter() - start_time) * 1000
            return "", {"latency_ms": total_latency, "tokens_per_sec": 0, "success": False, "error": "Timeout"}
        
        except Exception as e:
            total_latency = (time.perf_counter() - start_time) * 1000
            logger.error(f"측정 실패: {e}")
            return "", {"latency_ms": total_latency, "tokens_per_sec": 0, "success": False, "error": str(e)}
